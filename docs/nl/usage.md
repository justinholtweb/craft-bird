...
documententabel van Bird, er wordt een betaling op de factuur geregistreerd, en het
Moneybird-paneel op het bestelscherm vult zich.

Zodra de plugin is ingesteld hoef je daar niets voor te doen.
...
waar een bestelling Moneybird-JSON wordt, en het voorbeeld ís die plek — dus wat je vóór het boeken
...
|---|---|---|
| Klant in je thuisland | Binnenlands | Het toegewezen percentage |
| EU-onderneming, btw-nummer passend bij het land, geen btw berekend | Verlegd | Je tarief voor *btw verlegd* |
| EU-consument, OSS aan *(Pro)* | OSS | Het tarief van dat land |
| EU-consument, OSS uit | Thuistarief | Het toegewezen percentage |
| Buiten de EU | Export | Je exporttarief |

**Commerce bepaalt wat er berekend wordt; Bird bepaalt waar het geboekt wordt.** De belastingmotor
van Commerce kent je zones, je tarieven en je btw-nummercontroles al. Dat hier opnieuw afleiden zou
je webshop twee antwoorden geven die uit elkaar gaan lopen, en het antwoord dat de klant werkelijk
...
Bestellingen die al betaald waren toen je Bird installeerde blijven ongemoeid tot je erom vraagt.
...
met de hand afgevinkt. Zonder de webhook is het idee van je site over "betaald" wat de
...
**Status bij betaald** in als je wilt dat een betaalde factuur de Commerce-bestelling verzet.
...
{% if document and document.getIsBooked() %}
    ...
    {% if document.publicUrl %}
        <a href="{{ document.publicUrl }}">Bekijk je factuur</a>
    {% endif %}
{% endif %}
...
ratelimiet kan nooit de reden zijn dat een klant niet kon betalen. Wat je in plaats daarvan krijgt
is een documentregel in de staat `failed`, een reden erbij, en `bird/sync/retry`.
...
regel geschreven", zoekt de volgende poging de factuur op met
`GET /sales_invoices/find_by_reference/{reference}.json` en neemt die over in plaats van er een
tweede te boeken.
