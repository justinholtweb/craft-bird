...
slug: installation
order: 10
summary: Vereisten, installatie, een administratie koppelen en uw btw-tarieven toewijzen.
---
...
terug. Een plug-in installeren hoort nooit uit zichzelf in uw boekhouding te gaan schrijven.

## De administratie koppelen

Open **Instellingen → Plugins → Bird**.

1. **API-token.** Maak er een aan in Moneybird onder uw profiel → **Applicaties** → *API-tokens*.
Een token is net zo krachtig als uw wachtwoord, dus zet hem in een omgevingsvariabele en verwijs er
hier naar als `$MONEYBIRD_TOKEN` in plaats van de waarde in de database te plakken.
2. **Administratie.** Het nummer dat in de URL's van Moneybird zelf staat. Druk op **Administraties
tonen** als u liever uit een lijst kiest dan overtypt.
3. Druk op **Verbinding testen**. Dat roept `GET /administrations.json` aan — het enige endpoint dat
niet aan een administratie gebonden is — en vertelt u dus of het *token* werkt, ook als het id
verkeerd is.

## Uw btw-tarieven toewijzen

Dit is de stap die ertoe doet, en de enige waar Bird niet naar gaat raden.

1. Ga naar het onderdeel **Btw** en stelt u **Thuisland** in — het land waarvan u standaard de btw
in rekening brengt. Standaard is dat `NL`.
2. Druk op **Toewijzing voorstellen**. Bird leest de btw-tarieven die al in uw administratie staan
en stelt een tabel percentage → tarief-id voor. Neem die over in **Btw-tarieven**.
3. Zet het **Tarief voor btw verlegd** op het 0%-tarief in uw administratie dat *btw verlegd*
afdrukt, en het **Exporttarief** op het 0%-tarief dat u gebruikt voor verkopen buiten de EU.
...
roosters van een btw-aangifte terechtkomen, en het verkeerde kiezen levert een aangifte op die klopt
in de optelling en toch fout is.
...
eenmalig $ 99 met $ 49 per jaar verlenging, en voegt de onderdelen toe die pas gaan spelen zodra u
